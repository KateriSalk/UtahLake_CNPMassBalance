...
```{r, site-read-map}
ul_sites=wqTools::readWQP(type='sites', bBox=ul_bbox, siteType=c("Facility"), sampleMedia="Water") #Connection to WQP occasionally times out, function tries connection up to 10 times
...
```{r, read-wqp-data}
ul_nr=wqTools::readWQP(type='narrowresult', bBox=ul_bbox, siteType=c("Facility"), sampleMedia="Water", print=F)
...
```{r, write-raw}
write.csv(file=paste0("ul_Facilitydata_wqp_raw_",Sys.Date(),".csv"), ul_data, row.names=F)
```
...
```{r, write-processed}
write.csv(file=paste0("ul_Facilitydata_wqp_processed_",Sys.Date(),".csv"), ul_data, row.names=F)
```
