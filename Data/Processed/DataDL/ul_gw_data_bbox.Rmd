...
```{r, site-read-map}
ul_sites=wqTools::readWQP(type='sites', bBox=ul_bbox, siteType=c("Spring","Well"), sampleMedia="Water") #Connection to WQP occasionally times out, function tries connection up to 10 times
...
```{r, read-wqp-data}
ul_nr=wqTools::readWQP(type='narrowresult', bBox=ul_bbox, siteType=c("Spring","Well"), sampleMedia="Water", print=F)
...
```{r, write-raw}
write.csv(file=paste0("ul_wqp_gwdata_raw_",Sys.Date(),".csv"), ul_data, row.names=F)
```
...
```{r, check-unique}
check_data=ul_data[,c("MonitoringLocationIdentifier","datetime","SampleDepthValue","RelativeDepth","CharacteristicName","ResultSampleFractionText","ActivityStartDate","ActivityStartTime.Time")]
...
```{r, id-non-unique}
result_count_data=ul_data[,c("MonitoringLocationIdentifier","datetime","SampleDepthValue","RelativeDepth","ActivityStartDate","ActivityStartTime.Time","CharacteristicName","ResultSampleFractionText","ResultMeasureValue")]
...
```{r, write-processed}
write.csv(file=paste0("ul_wqp_gwdata_processed_",Sys.Date(),".csv"), ul_data, row.names=F)
```
