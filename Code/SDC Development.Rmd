...
#PTData_2 <- PTData$DateTime <- as.Date(PTData$DateTime,"%d/%m/%Y %H:%M")
#PTData2 <- mutate(PTData2, DateTime2 = "NA") 
...
```{r}
ggplot(data = PTData2, mapping = aes(x = year(DateTime3), y = PPSI)) +
...
#power.nls<-
nls(Flow.cfs~C*(PPSI+a)^n, data=PT_5578, start=list(C=1, a=0, n=1))
summary(Power.nls)
```


```{r}
nls <- nlsLM(data = PT_5578, formula = Flow.cfs~C*(PPSI+a)^n, start=list(a=1, n=0, C=2))
nls
coef(nls)
C <- coef(nls)["C"]
a <- coef(nls)["a"]
...
```
```{r}
with(PT_5578, plot(PPSI, Flow.cfs))
curve(C*(x+a)^n, add=TRUE)
```
```{r}
ggplot(data = PT_5578, mapping = aes(x = PPSI, y = Flow.cfs)) +
