...
*DST tz correction of PT datetime JV note - DWQ & WFWQC grab samples appear to also all come from WQP in MST. So conversion probably not necessary.
...
PT_2020 <- read.csv("./Data/Processed/PT2020_v2.csv")
...
```{r}
PTData2$DateRound <- round_date(PTData2$DateTime2, "15 minutes") 
```
...
trib$DateRound=as.POSIXct(trib$datetime, format = "%Y-%m-%d %H:%M:%S")
...
trib_flow <- trib %>%
  ...
  select(MonitoringLocationIdentifier:ResultSampleFractionText,ResultValueTypeName,
         OrganizationIdentifier:MonitoringLocationTypeName, LatitudeMeasure, 
         LongitudeMeasure, ResultMeasureValue, ResultMeasure.MeasureUnitCode, 
  ...
  group_by(MonitoringLocationIdentifier, DateRound, OrganizationIdentifier,ResultValueTypeName, 
...
colnames(trib_flow)[12:15] <- c("cfs", "mgd", "g.min", "m3.sec")
...
pt_plotly=plotly::plot_ly(data=PT_flow_match) %>% 
	plotly::add_trace(type="scatter", mode="markers", x=~PPSI, y=~Flow.cfs, name=~MLID)
htmlwidgets::saveWidget(pt_plotly, "pt_plotly.html")
writexl::write_xlsx(list(joined=PT_flow_match, all_grab_flow=trib_flow_long),path="joined_pt_data.xlsx")
```

# Compare all PSI to joined PSI by MLID
```{r}
boxplot(PTData2$PPSI, PT_flow_match$PPSI, ylab="PSI", names=c("All PT","Grab sample joined"))
plotly::plot_ly() %>% 
	plotly::add_boxplot(data=PTData2, y=~PPSI, x=~paste0(" ",MLID, " "), name="All") %>%
	plotly::add_boxplot(data=PT_flow_match, y=~PPSI, x=~paste0(" ",MLID, " "), name="Grab join") %>%
	plotly::layout(boxmode = "group", xaxis=list(title=""))
