...
trib2 <- read.csv("./Data/Processed/ul_Tribdata_wqp_processed_2020-12-17.csv")
# facility data
...
wshds2 <- read_sf("./Data/GIS/ulSubWshdsWGS84.shp")
...
# TP and PO4 measurements are always distinct (agencies may report one or the other). Combine into one column.
# 

#gw_wq_intersect <- gw_wq_intersect %>%
...
### Monthly conditions by watershed
...
```{r, fig.height = 3, fig.width = 5, echo = FALSE, message = FALSE, warning = FALSE}
...
# Lindon Drain
# Need to compare loads from WFWQC and DWQ. Generate each separately
...
temp <- data.frame(approx(loading_LindonDrain_WFWQC$load.TN.kgd, n = 12))
loading_LindonDrain_WFWQC$load.TN.kgd <- temp$y
temp <- data.frame(approx(loading_LindonDrain_WFWQC$load.TP.kgd, n = 12))
loading_LindonDrain_WFWQC$load.TP.kgd <- temp$y
temp <- data.frame(approx(loading_LindonDrain_WFWQC$load.TDP.kgd, n = 12))
loading_LindonDrain_WFWQC$load.TDP.kgd <- temp$y
temp <- data.frame(approx(loading_LindonDrain_WFWQC$load.TN.tonmo, n = 12))
loading_LindonDrain_WFWQC$load.TN.tonmo <- temp$y
temp <- data.frame(approx(loading_LindonDrain_WFWQC$load.TP.tonmo, n = 12))
loading_LindonDrain_WFWQC$load.TP.tonmo <- temp$y
temp <- data.frame(approx(loading_LindonDrain_WFWQC$load.TDP.tonmo, n = 12))
loading_LindonDrain_WFWQC$load.TDP.tonmo <- temp$y
...
temp <- data.frame(approx(loading_MillRace_WFWQCloads$load.TN.kgd, n = 12))
loading_MillRace_WFWQCloads$load.TN.kgd <- temp$y
temp <- data.frame(approx(loading_MillRace_WFWQCloads$load.TP.kgd, n = 12))
loading_MillRace_WFWQCloads$load.TP.kgd <- temp$y
temp <- data.frame(approx(loading_MillRace_WFWQCloads$load.TDP.kgd, n = 12))
loading_MillRace_WFWQCloads$load.TDP.kgd <- temp$y
temp <- data.frame(approx(loading_MillRace_WFWQCloads$load.TN.tonmo, n = 12))
loading_MillRace_WFWQCloads$load.TN.tonmo <- temp$y
temp <- data.frame(approx(loading_MillRace_WFWQCloads$load.TP.tonmo, n = 12))
loading_MillRace_WFWQCloads$load.TP.tonmo <- temp$y
temp <- data.frame(approx(loading_MillRace_WFWQCloads$load.TDP.tonmo, n = 12))
loading_MillRace_WFWQCloads$load.TDP.tonmo <- temp$y
...
#combine loading from different organizations
# REVISIT - had to average rather than using all measurements
loading_MillRace <- loading_MillRace_WFWQCloads %>%
  rbind(., loading_MillRace_UTAHDWQloads) %>%
  group_by(WS_Name, Month) %>%
  summarise(Flow.cfs = sum(Flow.cfs, na.rm = TRUE),
            load.TN.kgd = mean(load.TN.kgd, na.rm = TRUE), 
            load.TDN.kgd = mean(load.TDN.kgd, na.rm = TRUE),
...
# Dry Creek - Spanish Fork
# REVISIT - need to interpolate for January
...
loading_SpanishFork_DWQ <- trib_fac_sites_intersect_downstream %>%
  ungroup() %>%
  filter(OrganizationIdentifier == "UTAHDWQ_WQX") %>%
...
loading_SpanishFork_WFWQC <- trib_fac_sites_intersect_downstream %>%
  ungroup() %>%
  filter(OrganizationIdentifier == "WFWQC_UT") %>%
  filter(WS_Name == "Spanish Fork River") %>%
  group_by(WS_Name, Month) %>%
...
loading_AllWatersheds <- rbind(
  ...
  loading_LindonDrain_DWQ, loading_PowellSloughMajor, loading_ProvoRiver, 
  loading_MillRace, loading_SpringCreek, loading_HobbleCreek, loading_DryCreekSpanishFork, 
  loading_SpanishFork_DWQ, loading_4000SouthDrain, loading_BenjaminSlough, 
...
### Nutrient concentrations across watersheds
...
ggplot(subset(trib_fac_sites_intersect_downstream, WS_Name == "Lehi Spring Creek"),
  ...
  geom_boxplot(position = position_dodge(preserve = "single"), alpha = 0.8) +
  geom_hline(yintercept = 185, lty = 2, color = "#f27162") +
  geom_hline(yintercept = 318, lty = 2, color = "#62197d") +
  stat_summary(fun.data = give.n, geom = "text", size = 3,
                  position = position_dodge(width = 0.75)) +
...
ggplot(subset(trib_fac_sites_intersect_downstream, WS_Name == "Lehi Spring Creek"),
       aes(x = as.factor(Month), y = TP, fill = MonitoringLocationIdentifier, 
           color = Backwater)) +
  geom_boxplot(position = position_dodge(preserve = "single"), alpha = 0.8) +
  geom_hline(yintercept = 5, lty = 2, color = "#f27162") +
  geom_hline(yintercept = 10, lty = 2, color = "#62197d") +
  stat_summary(fun.data = give.n, geom = "text", size = 3,
                  position = position_dodge(width = 0.75)) +
