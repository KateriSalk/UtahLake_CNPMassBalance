...
### Examine detection and reporting limits
```{r}
detection <- trib_fac_sites_intersect_downstream %>%
  left_join(trib)
```
...
# Spanish Fork
# Need to compare loads from WFWQC and DWQ. Generate each separately
# TN  concentrations are below reporting limit for WFWQC --> use only DWQ samples
...
loading_SpanishFork_WFWQC <- trib_fac_sites_intersect_downstream %>%
  ...
  filter(OrganizationIdentifier == "WFWQC_UT") %>%
  filter(WS_Name == "Spanish Fork River") %>%
  mutate(load.TN.kgd = case_when(OrganizationIdentifier == "WFWQC_UT" ~ NA_real_,
                                 TRUE ~ load.TN.kgd)) %>%
  group_by(WS_Name, Month) %>%
  summarise(Flow.cfs = mean(Flow.cfs, na.rm = TRUE),
            load.TN.kgd = mean(load.TN.kgd, na.rm = TRUE), 
