...
loading_LindonDrain_dmr <- dmr_load %>%
  ...
  mutate(WS_Name = "Lindon Drain", 
         ...
         load.TN.tonmo = NA,
         load.TDN.tonmo = NA,
         load.TP.tonmo = load.TP.tonmo,
         load.TDP.tonmo = NA,
         load.TOC.tonmo = NA,
         load.DOC.tonmo = NA) %>%
  select(WS_Name, Month, Flow.cfs, load.TN.kgd, load.TDN.kgd, load.TP.kgd,
         load.TDP.kgd, load.TOC.kgd, load.DOC.kgd, Flow.acftmo, load.TN.tonmo,
         load.TDN.tonmo, load.TP.tonmo, load.TDP.tonmo, load.TOC.tonmo, load.DOC.tonmo)
...
loading_LindonDrain_DWQ <- loading_LindonDrain_DWQ %>%
  rbind(., loading_LindonDrain_dmr) %>%
  group_by(WS_Name, Month) %>%
...
ggplot(loading_LindonDrain_DWQ, aes(x = Month, y = Flow.acftmo)) +
  geom_line() +
  geom_line(data = loading_LindonDrain_WFWQC, aes(x = Month, y = Flow.acftmo),
            color = "blue")
ggplot(loading_LindonDrain_DWQ, aes(x = Month, y = load.TP.tonmo)) +
  geom_line() +
  geom_line(data = loading_LindonDrain_WFWQC, aes(x = Month, y = load.TP.tonmo),
            color = "blue")
