...
give.n <- function(x){
   return(c(y = (max(x) + 0.1*(max(x))), label = length(x)))
}
```
...
```{r}
# TN
ggplot(trib_fac_sites_intersect_downstream, aes(x = ActivityStartDate, y = cfs, shape = SiteType,
                                                color = OrganizationIdentifier)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_point(aes(y = mgd), alpha = 0.7, size = 2) +
  geom_point(aes(y = g.min), alpha = 0.7, size = 2) +
  geom_point(aes(y = m3.sec), alpha = 0.7, size = 2) +
  facet_wrap(vars(WS_Name), ncol = 3) +
  scale_y_log10() +
