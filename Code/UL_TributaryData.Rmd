...
p_et_summary <- p_et_monthly %>%
  group_by(Month) %>%
  summarise(precip.mm = mean(precip.mm), 
            et.mm = mean(et.mm)) %>%
  #multiply by lake area (95,000 acres)
  mutate(precip.acftmo = precip.mm * 95000 / 304.8, 
         et.acftmo = et.mm * 95000 / 304.8)
# write.csv(p_et_summary, file = "./Data/Processed/hydrology_precip_et_monthly.csv",
#           row.names = FALSE)
