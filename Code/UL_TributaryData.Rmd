...
The different units don't seem to match up in value when we convert them to cfs
```{r}
...
test <- full_join(trib_fac_wq, trib_flow) 

```

#### Preliminary C-Q plots
```{r}
ggplot(test, aes(x = cfs, y = TN)) +
  geom_point()
```
...
* UTAHDWQ_WQX-4996030 is an upstream site
