...
trib_flow <- trib %>%
  ...
  filter(!is.na(datetime), 
         ...
         ResultMeasure.MeasureUnitCode %in% c("ft", "ft3/s")) %>%
  select(MonitoringLocationIdentifier:LongitudeMeasure, ResultMeasureValue, ResultMeasure.MeasureUnitCode) %>%
  select(-SampleDepthValue, -RelativeDepth, -ResultSampleFractionText, 
         -MonitoringLocationTypeName, -ResultMeasure.MeasureUnitCode) %>%
  mutate(date = date(datetime)) %>%
  group_by(MonitoringLocationIdentifier, date, CharacteristicName, 
           OrganizationIdentifier, OrganizationFormalName, MonitoringLocationName,
...
colnames(trib_flow)[8:10] <- c("Q.cfs", "gageheight.ft", "Q.daily.cfs")
...
trib_fac_sites_wsds <- trib_fac_sites_intersect %>%
  ...
  distinct() %>%
  arrange(WS_Name)
write.csv(trib_fac_sites_wsds, file = "./Data/Processed/trib_fac_sites.csv",
          row.names = FALSE)
...
```{r}
ggplot(trib_flow, aes(x = date, y = Q.cfs)) +
  geom_point() +
  scale_y_log10() +
  facet_wrap(vars(MonitoringLocationName))
```
