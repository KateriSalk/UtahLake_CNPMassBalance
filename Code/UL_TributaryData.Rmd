...
trib2 <- read.csv("./Data/Processed/ul_Tribdata_wqp_processed.csv")
...
The different units don't seem to match up in value when we convert them to cfs. May be due to site types. Revisit.
```{r}
...
trib_fac_flow <- rbind(trib_flow, fac_flow)
...
# PO4.t
ggplot(trib_fac_sites_intersect_downstream, aes(x = ActivityStartDate, y = PO4.t, shape = SiteType,
                                                color = OrganizationIdentifier)) +
  geom_point(alpha = 0.7, size = 2) +
  facet_wrap(vars(WS_Name), ncol = 3) +
...
# PO4.d
ggplot(trib_fac_sites_intersect_downstream, aes(x = ActivityStartDate, y = PO4.d, shape = SiteType,
                                                color = OrganizationIdentifier)) +
  geom_point(alpha = 0.7, size = 2) +
  facet_wrap(vars(WS_Name), ncol = 3) +
...
```{r}
# TN
ggplot(trib_fac_sites_intersect_downstream, aes(x = cfs, y = TN, shape = SiteType,
                                                color = OrganizationIdentifier)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_point(aes(x = mgd), alpha = 0.7, size = 2) +
  geom_point(aes(x = g.min), alpha = 0.7, size = 2) +
  geom_point(aes(x = m3.sec), alpha = 0.7, size = 2) +
  facet_wrap(vars(WS_Name), ncol = 3) +
  scale_x_log10() +
