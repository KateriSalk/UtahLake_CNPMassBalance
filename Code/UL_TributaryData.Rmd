...
ul_level_processed <- ul_level %>%
  mutate(Year = year(Date), 
         Month = month(Date), 
         SurfaceArea.ac = as.numeric('Surface.Area.(m2)')/4046.86) %>%
...
dmr_flow <- dmr %>%
  ...
  filter(Year >= 2015) %>%
  filter(NPES_Name %in% c("OREM CITY CORP", "PAYSON CITY CORP", "PROVO CITY CORP",
                          "SALEM CITY CORP", "SPANISH FORK CITY CORP", 
...
ggplot(subset(trib_fac_sites_intersect_downstream_early, WS_Name == "Mill Race"),
...
# separate WFWQC measurements. These represent the total load (both individually), whereas the DWQ data is two sites that need to be combined.
...
# calculate sum of 6540 and 6566. These represent the total load.
